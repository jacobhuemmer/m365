...
`--allow=` denies all real workload writes. These guards apply to workload writes; commands that save or download files can still write local paths. `--exact-recipients` validates `--to` on mail and Teams sends, not mail `--cc` or calendar attendees. To message yourself with exact matching enabled, use `--note-to-self` instead of your own email address or user ID. Teams user-ID matching requires the signed-in account to be identifiable among the chat members; otherwise it fails closed.
